...
<head>
  ...
  @if(isset(Auth::user()->name))
    <footer class="site-footer">
      <div class="site-footer-legal">© {{ date('Y') }} <a title="Dashboard" href="{{ route('dashboard') }}">Febrasgo</a></div>
    </footer>
  @endif
  ...
  <script type="text/javascript">
    jQuery(document).ready(function () {
      ...
      @endforeach
      qtdeNotifications();
      setInterval(qtdeNotifications, 10000);
    });


    function qtdeNotifications() {
      $.ajax({
        dataType: 'json',
        url: '{{ URL::asset("getqtdenews") }}',
        type: 'GET',
        async: true,
        success: function (data) {
          if (data.success) {
            $('#qtdeNotifications').html(data.data).fadeIn();
          }
        }
      });
    }
